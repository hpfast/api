...
function expandConcepts(results, expand, callback) {
  if (expand) {
    graph.expand(results, function(err, concepts) {
      callback(err, concepts);
    });
  } else {
    callback(null, results);//maybe we would want to process results to format geojson.js can consume ...
  }
}

function related(esParams, relations, direction, expand, callback) {
  _([esParams])
    .nfcall([])
    .series()
    .errors(function(err) {
    ...
    .apply(function(ids) {
      graph.related(ids, relations, direction, function(err, ids) {
        if (err) {
          ...
          expandConcepts(ids,expand, callback);
        }
      });
    });
}
...
module.exports = function(query, callback) {
  ...
  if (hasSearchParam && hasRelatedParam && hasRelatedSearchParam) {
    ...
    related(esQueryFrom, query.related, 'to', query.expand, callback);

  } else if (hasRelatedParam && hasRelatedSearchParam) {
    related(esQueryTo, query.related, 'from', query.expand, callback);
    ...
    _([esQueryFrom])
      ...
      .toArray(function(ids) {
        expandConcepts(ids,query.expand,callback);
      });
